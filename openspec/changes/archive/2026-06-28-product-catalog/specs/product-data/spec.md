...
The system MUST validate that parsed data contains a `productos` array and each item has `nombre`, `categoria`, and `carpeta_imagenes` as non-empty strings.
...
- AND each object preserves all YAML fields including `presentaciones` and `conservacion`
